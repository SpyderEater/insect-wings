            ...
            for x in range(radius, width - radius):
                ...
                for dy in range(-radius, radius + 1):
                    for dx in range(-radius, radius + 1):
                        pixel = img[y + dy, x + dx]
                        brightness = sum(int(c) for c in pixel) / 3
                        window.append(brightness)
                
                avg_brightness = sum(window) / total_pixels
                
                if avg_brightness > threshold:
                    new_img[y, x] = [255, 255, 255]
                else:
                    window.sort()
                    pixel_result = int(window[median_idx])
                    
                    if pixel_result == 0 and is_debug:
                        debug_log_pixel_avg(log_file, y, x, avg_brightness)
                    
                    new_img[y, x] = [pixel_result, pixel_result, pixel_result]
    finally:
        if log_file:
            log_file.close()
